...
class StellarData(DataObject):
    ...
    def __init__(self, density: float, density_error: float, radius: float, radius_error: float, mass: float, mass_error: float):
        """
    ...
    def from_database(cls, tic: int):
        ...
        """
        try:
            astropy_star_datatable = Catalogs.query_object(f"TIC {tic}", catalog="TIC", radius=0.001)
            ...
            logger.warning(f"No stellar data associated with TIC {tic}. Eccentricity post-processing step may fail.")
            star = dict()
        return cls(
    ...
    def density_data_present(self):
        return np.all(np.isfinite([self.density, self.density_error]))

    def __str__(self):
        m_str = f"mass = {self.mass:0.2f} ± {self.mass_error:0.2f} (Msun)"
        r_str = f"radius = {self.radius:0.2f} ± {self.radius_error:0.2f} (Rsun)"
        rho_str = f"density = {self.density:0.2f} ± {self.density_error:0.2f} (solar)"
        return "\n".join["Stellar Info:", m_str, r_str, rho_str]
